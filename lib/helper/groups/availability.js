export const calcBuddyAvailabilityMsg = (group) => {
  ...
  if (buddyPositionFilled) {
    buddyAvailabilityMsg = (
      <p className="card-group-unavailable margin-tp-12">
        Buddy position filled
      </p>
    );
  } else {
    let NSpotsAvailable;
    ...
    buddyAvailabilityMsg = (
      <p className="card-group-available margin-tp-12">
...
export const calcBuddyAvailabilityCTA = (group, currentUser) => {
  ...
  if (buddyPositionFilled) {
    ...
    // else if (currentUser && currentUser._id && currentUser._id.length === 0) {
    //   buddyAvailabilityCTA = true;
    // }
    else if (currentUser && currentUser._id && currentUser._id.length > 0) {
      if (
        ...
        // the current user is not already in the group
        buddyAvailabilityCTA = true;
      }
    }
  }
...
export const calcMentorAvailabilityCTA = (group, currentUser) => {
  ...
  if (!mentorRequired) {
    ...
    // console.log(
    //   `mentorRequired && !mentorPositionFilled: ${
    //     mentorRequired && !mentorPositionFilled
    //   }`
    // );
